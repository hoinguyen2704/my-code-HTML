...
public class UserService implements IUserService {
    ...
    @Override
    public List<StaffResponseDTO> getStaffResponseDTOS(Map<Long, String> listStaffs, Map<Long, String> listStaffAssigned) {
        List<StaffResponseDTO> staffAssignment = new ArrayList<>();
        for(Map.Entry<Long, String> entry : listStaffs.entrySet()){
            Long staffId = entry.getKey();
            StaffResponseDTO staff = new StaffResponseDTO();
            staff.setFullName(entry.getValue());
            staff.setStaffId(entry.getKey());
            if (listStaffAssigned.containsKey(staffId)){
